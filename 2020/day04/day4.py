...
class Passport:
    def __init__(self, data: str):
        ...
        if {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"}.issubset(
            self.features.keys()
        ):
            self.valid = True
        ...
        if self.valid:
            ...
            hgt_valid = (
                self.features.get("hgt").endswith("cm")
                and 150 <= int(self.features.get("hgt")[:-2]) <= 193
            ) or (
                self.features.get("hgt").endswith("in")
                and 59 <= int(self.features.get("hgt")[:-2]) <= 76
            )
            hcl_valid = bool(re.fullmatch(r"#[\d-f]{6}", self.features.get("hcl")))
            ecl_valid = self.features.get("ecl") in [
                "amb",
                "blu",
                "brn",
                "gry",
