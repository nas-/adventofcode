...
def occupied_adjacent(a, x, y):
    places = [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1), (x + 1, y + 1), (x + 1, y - 1), (x - 1, y + 1),
              (x - 1, y - 1)]
    return sum(1 for place in places if
               0 <= place[0] <= (len(a) - 1) and 0 <= place[1] <= (len(a[0]) - 1) and a[place[0]][place[1]] == '#')
...
def occupied_non_adjacent(a, x, y):
    ...
    for place in vectors:
        ...
        while not found:
            ...
            if 0 <= sum_values[0] <= (len(a) - 1) and 0 <= sum_values[1] <= (len(a[0]) - 1):
